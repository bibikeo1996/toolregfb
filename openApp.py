...
def is_app_installed(adb_path, package_name):
    result = subprocess.run(
        [adb_path, "shell", "pm", "list", "packages"], capture_output=True, text=True
    )
...
def open_bluestacks_and_install_apk(apk_path):
    ...
    try:
        ...
        response = requests.get(
            "http://httpbin.org/ip", proxies={"http": proxy, "https": proxy}
        )
        ...
        subprocess.run(
            [adb_path, "shell", "input", "swipe", "116", "826", "116", "926", "300"]
        )
        time.sleep(1)
        subprocess.run(
            [adb_path, "shell", "input", "swipe", "236", "826", "236", "926", "300"]
        )
        time.sleep(1)
        subprocess.run(
            [adb_path, "shell", "input", "swipe", "356", "826", "356", "2826", "300"]
        )
        time.sleep(1)
...
# Xuất sang Excel
def export_to_excel(file_path, user_id, password, cookie):
    # Nếu file Excel chưa tồn tại, tạo mới file và thêm tiêu đề
    if not os.path.exists(file_path):
        workbook = openpyxl.Workbook()
        sheet = workbook.active
        sheet.title = "Facebook Accounts"
        sheet.append(["User ID", "Password", "Cookie"])  # Tiêu đề
    else:
    ...
    sheet.append([user_id, password, cookie])
...
# Remove ứng dụng
def uninstall_app(adb_path, package_name):
    try:
...
apk_path = r"F:\Projects\toolregfb\fblite.apk"  # Đổi đường dẫn tới APK bạn muốn cài
open_bluestacks_and_install_apk(apk_path)

if __name__ == "__main__":
    # Đường dẫn tới adb.exe
    adb_path = r"F:\Projects\platform-tools\adb.exe"

    # Package name của ứng dụng bạn muốn gỡ cài đặt
    package_name = "com.facebook.lite"

    # File Excel xuất thông tin
    output_file = "app_data.xlsx"

    # Giả sử bạn có thông tin tài khoản, mật khẩu và cookie
    user_id = "kz46zbc7umb@gmail.com"
    password = "matkhau@1123"
    cookie = "example_cookie_value"

    # Xuất dữ liệu vào file Excel
    export_to_excel(output_file, user_id, password, cookie)

    # Sau khi xuất dữ liệu thành công, tiến hành gỡ cài đặt
    uninstall_app(adb_path, package_name)
